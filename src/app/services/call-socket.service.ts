...
export class CallSocketService {
  ...
  onJobChanged(): Observable<any> {
    return new Observable(observer => {
      ...
      this.socket.on('materialJob:changed', handler);

      // cleanup ตอน unsubscribe
      return () => {
        this.socket.off('materialJob:changed', handler);
      };
    });
  }

  onStoreChange(): Observable<any> {
    return new Observable(observer => {
        const handler = (data: any) => observer.next(data);
  
        this.socket.on('materialStore:changed', handler);
  
        // cleanup ตอน unsubscribe
        return () => {
          this.socket.off('materialStore:changed', handler);
        };
      });
  }
